...
- **Test 2 (Row count)**: dbt-expectations macro `expect_table_row_count_to_equal_other_table` on `dim_account` with `compare_model: ref('_expected_dim_account_periods')`. The model `_expected_dim_account_periods` (view) computes the expected period count from `int_account_status_history` (one row per created/reopened). Build it before testing: `dbt run --select _expected_dim_account_periods` (or run all marts).
