...
| Asset | Purpose |
|-------|--------|
| **`models/marts/diagnose_dim_account_current_breakdown.sql`** | View: for each `current_count` (0, 2, 3, …), how many accounts. Run with `dbt show --select diagnose_dim_account_current_breakdown`. |
